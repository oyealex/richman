...
系统 SHALL 提供 `BoardWidget` Textual widget，接收 `GameSnapshot`、`TuiLayoutGeometry` 和可选的 `terminal_size: tuple[int, int] | None`，按 `position_rects` 绝对定位渲染所有棋盘格和中心信息区。

#### Scenario: BoardWidget renders all cells from geometry

- **WHEN** 向 BoardWidget 传入 `GameSnapshot` 和合法的 `TuiLayoutGeometry`（`is_terminal_sufficient=True`）
...
- **WHEN** `TuiLayoutGeometry.is_terminal_sufficient` 为 `False` 且 `terminal_size` 参数非 None
...
- **WHEN** 渲染一个 cell，`GameSnapshot.public_players` 中某玩家 position 等于该 cell 的 position
